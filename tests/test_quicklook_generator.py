...
def test_generate_quicklook_instance():
    ...
    assert isinstance(generate_instrument_quicklook(mag_file), MagQuicklookGenerator)
    assert isinstance(generate_instrument_quicklook(idex_file), IdexQuicklookGenerator)
